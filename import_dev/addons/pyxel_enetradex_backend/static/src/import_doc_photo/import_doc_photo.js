...
import { buildDocScanOverlay } from "@pyxel_enetradex_backend/scan_overlay/scan_overlay";

const SCAN_DS = `http://${window.location.hostname}:8900`;
...
export class ImportDocPhoto extends Component {
    ...
    setup() {
        ...
        onMounted(() => this._checkScanner());
        onWillUnmount(() => this.closeCamera());
    }

    async _checkScanner() {
        try {
            const ctrl = new AbortController();
            const tid = setTimeout(() => ctrl.abort(), SCAN_TIMEOUT);
            const resp = await fetch(`${SCAN_DS}/capabilities`, { signal: ctrl.signal });
            clearTimeout(tid);
            if (resp.ok) {
                const data = await resp.json();
                this.state.scannerAvailable = data.ok === true || !!data.name;
            }
        } catch (_) {
            this.state.scannerAvailable = false;
        }
    }
    ...
    async openScanner() {
        ...
        const docLabel = record.data?.document_label || "Documento";
        const overlay = buildDocScanOverlay(SCAN_DS);
        overlay.open(docLabel, async (pdf_b64, filename) => {
            this.state.busy = true;
            try {
                await this.orm.call(this.model, "upload_pdf_b64", [[this.docId], pdf_b64, filename]);
                await record.load();
                this.notification.add(_t("Documento escaneado y subido correctamente."), { type: "success" });
            } catch (e) {
                this.notification.add(`Error al subir el escaneo: ${e.message}`, { type: "danger" });
            } finally {
    ...
    async onPdfSelected(ev) {
        ...
        if (!ALLOWED.includes(ext)) {
            this.notification.add(_t("Formato no permitido. Use PDF, Excel o Word."), { type: "danger" });
            return;
        }
        if (file.size > 10 * 1024 * 1024) {
            this.notification.add(
                _t("El archivo supera 10 MB (%.1f MB). Comprime el archivo.").replace("%.1f", (file.size / 1024 / 1024).toFixed(1)),
                { type: "danger" }
            );
            return;
        }
        ...
        try {
            ...
            await record.load();
            this.notification.add(_t("Archivo subido correctamente."), { type: "success" });
        } catch (e) {
            this.notification.add(_t("Error al subir el archivo."), { type: "danger" });
            throw e;
        } finally {
            this.state.busy = false;
        }
