...
class PyxelImportDocument(models.Model):
    ...
    bl_number = fields.Char(related='purchase_order_id.bl_number', string="BL / AWB", store=False)
    ...
    def _compute_portal_state(self):
        for d in self:
            ...
            if not d.attachment_id and d.commercial_state != 'approved':
                ...
                # 'passed', 'doubt', 'unavailable' y tambien 'rejected'
                # -> revision manual activa.
                # La IA INFORMA, NO BLOQUEA: antes su 'rejected' dejaba el
                # documento cerrado sin que el comercial pudiera dictaminar.
                state = 'in_review'
                if d.ai_state == 'rejected':
                    # el motivo de la IA se ensena hasta que una persona decida
                    reason = d.ai_reason
    ...
    def write(self, vals):
        ...
        if 'attachment_id' in vals and vals.get('attachment_id'):
            ...
            for d in run_ai_for:
                d._run_ai()
        # 'rejected' incluido: tambien habilita al comercial. Es informacion.
    ...
    def action_commercial_approve(self):
        for d in self:
            ...
            # Antes aqui se cortaba con "La IA rechazo el documento; corrigelo
            # o reemplazalo antes de aprobar". El comercial tiene el documento
            # delante y la IA se equivoca a menudo con documentos legitimos:
            # el dictamen se le ensena, pero la decision es suya.
    ...
    def build_oc_expediente(self, purchase_orders):
        ...
        for po in purchase_orders:
            ...
            if self.search_count([('purchase_order_id', '=', po.id)]):
                continue
            # Bloque de cliente que generó esta OC.
            # Primero por purchase_order_id (caso normal); si no, por proceso+cliente
            # (caso legacy donde el bloque no quedó enlazado a la OC).
            block = ClientBlock.search([('purchase_order_id', '=', po.id)], limit=1)
            if not block and po.customer_id:
                block = ClientBlock.search([
                    ('process_id', '=', po.importation_id.id),
                    ('customer_id', '=', po.customer_id.id),
                ], limit=1)
            seq = 10
