...
LABEL_TO_DOCVAL = {
    ...
    'Identificación fiscal única RC05': 'identificacion_fiscal',
...
DOC_CATALOG = {
    'Pymes': [
        ...
        ('carnet_nit', 'Identificación fiscal única RC05', True),
    ...
    'Proveedor': [
        ...
        ('perfil_compania', 'Perfil de la compañía', True),
        ('solvencia_financiera', 'Solvencia o reporte financiero', True),
    ],
}
...
DEFAULT_CHECKS = {
    ...
    'perfil_compania':        [('Describe la actividad de la empresa', 'medio'),
                               ('Datos de contacto y ubicación', 'medio')],
    'solvencia_financiera':   [('Estados financieros o balance visible', 'alto'),
                               ('Fecha de emisión reciente', 'medio'),
                               ('Firma o sello del emisor', 'alto')],
}
...
class PyxelLeadDocument(models.Model):
    ...
    is_pdf_attachment = fields.Boolean(
        compute='_compute_is_pdf_attachment', string='Es PDF')

    @api.depends('attachment_id.mimetype')
    def _compute_is_pdf_attachment(self):
        for rec in self:
            rec.is_pdf_attachment = (
                rec.attachment_id and
                rec.attachment_id.mimetype == 'application/pdf'
            )
    ...
    def _compute_portal_state(self):
        for d in self:
            ...
            if not d.attachment_id and d.commercial_state != 'approved':
                ...
                state = 'validating'
            elif d.ai_state in ('passed', 'doubt', 'unavailable', 'rejected'):
                # La IA INFORMA, NO BLOQUEA. Su 'rejected' entra igual en el
                # circuito de revision humana. Antes se cortaba aqui: el
                # documento quedaba 'rejected' sin que la abogada ni el
                # comercial llegaran siquiera a verlo, y el cliente no podia
                # avanzar. Se comprobo que el motor de reglas rechaza una
                # proporcion alta de documentos legitimos (el mismo PDF que
                # da 'no_apto' por palabras clave lo clasifica el CNN como
                # correcto con 99% de confianza), asi que su veredicto no
                # puede ser la ultima palabra.
                # 'unavailable' = la IA no respondio; flujo manual sin criterio.
                if d.ai_state == 'rejected':
                    # se ensena el motivo de la IA hasta que una persona dictamine
                    reason = d.ai_reason
    ...
    def write(self, vals):
        ...
        # 'rejected' incluido: un rechazo de la IA tambien habilita a la
        # abogada. Es informacion para ella, no un veto.
        if 'ai_state' in vals and vals['ai_state'] in ('passed', 'doubt', 'unavailable', 'rejected'):
            for d in self:
    ...
    def upload_pdf_b64(self, b64_data, filename=None):
        """Recibe un archivo en base64 desde el widget JS, valida y lo adjunta.
        Acepta PDF, Excel (.xls/.xlsx) y Word (.doc/.docx)."""
        self.ensure_one()
        ALLOWED_EXT = {'.pdf', '.xls', '.xlsx', '.doc', '.docx'}
        MIME_MAP = {
            '.pdf':  'application/pdf',
        ...
        if ext not in ALLOWED_EXT:
            raise ValidationError(_("Formato no permitido. Use PDF, Excel (.xls/.xlsx) o Word (.doc/.docx)."))
        raw = base64.b64decode(b64_data)
        if len(raw) > 10 * 1024 * 1024:
            raise ValidationError(
                _("El archivo no puede superar 10 MB (pesa %.1f MB).") % (len(raw) / 1024 / 1024))
        att = self.env['ir.attachment'].sudo().create({
            'name': fname,
            'datas': b64_data,
            'res_model': self._name,
    ...
    def action_lawyer_approve(self):
        for d in self:
            # 'rejected' NO impide aprobar: la abogada puede estar viendo un
            # documento perfectamente valido que las reglas de texto no
            # supieron reconocer. Solo se espera a que la IA termine.
            if d.ai_state in ('validating', 'pending'):
    ...
    def action_lawyer_reopen(self):
        for d in self:
            if d.ai_state not in ('validating', 'pending'):
