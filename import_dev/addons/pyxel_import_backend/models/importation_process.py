...
class ImportationProcess(models.Model):
    ...
    def write(self, vals):
        ...
        for record in self:
            # packing_list vive en purchase.order; solo actuar si el campo existe en este modelo
            if 'packing_list_filename_date' in self._fields:
                if ('packing_list' in vals and not vals.get('packing_list')) or (
                        not getattr(record, 'packing_list', None) and 'packing_list' not in vals):
