...
class ImportDocumentPage(models.Model):
    _name = 'pyxel.import.document.page'
    _description = 'Página fotográfica del documento de importación'
    _order = 'page_number'

    document_id = fields.Many2one('pyxel.import.document', required=True, ondelete='cascade')
    ...
    def create_from_b64(self, document_id, images_b64):
        doc = self.env['pyxel.import.document'].browse(document_id)
        ...
        if not force:
            ...
            if bad:
                ...
                raise ValidationError(
                    _("Las páginas %s tienen calidad insuficiente.") % nums)
        try:
            from PIL import Image as PILImage
            imgs = []
            for page in pages:
