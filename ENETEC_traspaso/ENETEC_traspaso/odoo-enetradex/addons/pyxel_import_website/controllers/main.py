...
class WebsiteForm(form.WebsiteForm):
    ...
    def _get_partner_data(self, kwargs):
        partner_data = {
                ...
                "objeto_social": kwargs.get("objeto_social"),
    ...
    def website_form(self, model_name, **kwargs):
        ...
        if model_name == "crm.lead" and tipo_registro == "accreditation":
            ...
            for file_key in file_keys:
                ...
                # Guarda de robustez: saltar valores que no sean archivos reales
                # (inputs vacíos/no enviados) para no crear adjuntos vacíos ni romper.
                if not hasattr(file, 'seek') or not getattr(file, 'filename', None):
                    continue
...
class ControllerTest(http.Controller):
    ...
    def controller_register(self, **kw):
        # Form viejo DEPRECADO: la acreditación y la solicitud de importación van
        # ahora por el wizard /en/wizard. Se redirige para mantener un único punto
        # de entrada y evitar que este form vuelva a pedir acreditación.
        if request.env.user.id == request.env.ref('base.public_user').id:
            return request.redirect("/web/login?redirect=/en/wizard")
        if kw.get('type') == 'import':
            return request.redirect('/en/wizard?op=1')
        return request.redirect('/en/wizard')
