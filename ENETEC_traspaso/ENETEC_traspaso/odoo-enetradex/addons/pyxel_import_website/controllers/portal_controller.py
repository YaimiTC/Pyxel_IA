...
# Tipos de OV internos que el cliente NO debe ver en su portal
_INTERNAL_SO_TYPES = ('evaluation_initial', 'evaluation_final')
...
class Portal(CustomerPortal):
    ...
    def _en_dashboard_values(self):
        ...
        # Expediente de acreditación para el widget del dashboard
        ...
        dash_acc_docs = []
        if lead:
            docs = lead.accreditation_document_ids.filtered(lambda d: d.is_required)
            dash_acc_docs = [{'label': d.document_label, 'state': d.portal_state} for d in docs]
    ...
    def _prepare_portal_layout_values(self):
        ...
        new_quotations_count = 0  # purchase.order para proveedor
        new_orders_count = 0      # sale.order para cliente
        new_invoices_count = 0    # facturas
        new_importaciones_count = 0  # importaciones activas
        ...
        if is_internal_user:
            ...
            new_orders_count = request.env['sale.order'].sudo().search_count([
                ('state', 'in', ('sale', 'done')),
                ('order_type', 'not in', list(_INTERNAL_SO_TYPES)),
                ('is_cost_order', '=', False),
            ])
            ...
            # Incidencia 3: excluir OV de evaluación y costos — el cliente solo ve su OV de importación
            new_orders_count = request.env['sale.order'].sudo().search_count([
                ('state', 'in', ('sale', 'done')),
            ...
            # Incidencia 5: contar facturas confirmadas (posted), no borradores
            new_invoices_count = request.env['account.move'].sudo().search_count([
                ('state', '=', 'posted'),
                ('move_type', '=', 'out_invoice'),
                ('partner_id', '=', business_partner_id),
            ])
            # Incidencia 2: contar TODAS las importaciones activas, sin filtro de etapa
            new_importaciones_count = request.env['importation.process'].sudo().search_count([
            ...
            # Incidencia 4: incluir OC en estado sent y purchase, no solo draft
            new_quotations_count = request.env['purchase.order'].sudo().search_count([
                ('state', 'in', ('draft', 'sent', 'purchase')),
                ('partner_id', '=', business_partner_id),
            ])
            ...
            # Incidencia 2: contar TODAS las importaciones activas
            new_importaciones_count = request.env['importation.process'].sudo().search_count([
        ...
        values.update({
            ...
            'contact_type': contact_type.type_of_contact if contact_type else False,
        })

        return values

    # ===================== Incidencia 1 — /my/notifications =====================
    @http.route(['/my/notifications'], type='http', auth='user', website=True)
    def portal_notifications(self, **kw):
        """Redirige al dashboard; pendiente de implementar pantalla propia."""
        return request.redirect('/my/home')

    # ===================== Incidencia 3 — filtrar OV internas en /my/orders =====================
    @http.route(['/my/orders', '/my/orders/page/<int:page>'], type='http', auth='user', website=True)
    def portal_my_orders(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        """Hereda el listado estándar y excluye OV de evaluación y costos."""
        # Llamamos al padre para obtener la respuesta estándar
        response = super().portal_my_orders(
            page=page, date_begin=date_begin, date_end=date_end, sortby=sortby, **kw)
        # Si el padre devolvió un render con qcontext, filtramos las orders
        if hasattr(response, 'qcontext') and 'orders' in response.qcontext:
            response.qcontext['orders'] = response.qcontext['orders'].filtered(
                lambda o: o.order_type not in _INTERNAL_SO_TYPES and not o.is_cost_order
            )
        return response

    # ===================== Incidencia 4 — OC del proveedor =====================
    @http.route(['/my/compras', '/my/compras/page/<int:page>'], type='http', auth='user', website=True)
    def portal_my_compras(self, page=1, sortby=None, **kw):
        """Lista de órdenes de compra del proveedor."""
        env = request.env
        ...
        return request.render('pyxel_import_website.portal_my_compras', values)
