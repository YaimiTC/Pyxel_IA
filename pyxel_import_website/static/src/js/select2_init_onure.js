(function ($) {
    $(document).ready(function () {
        ...
        const $selector = $('#productOnure');

        $selector.select2({
            width: '50%',
            placeholder: "Seleccione los productos onure",
            language: {
                noResults: function () {
        ...
        let electronicos_de_importacion = JSON.parse($selector.attr("data-electronicos") || "[]");
        let ElectronicSelectedProducts  = JSON.parse($selector.attr("data-electronicos-selected") || "[]");

        // Función para cargar las opciones en el select
        function loadSelect2Options(products, ElectronicSelectedProducts) {
            $selector.empty(); // Limpiar las opciones existentes
            products.forEach(function (product) {

                let isSelected = ElectronicSelectedProducts.includes(parseInt(product.id));
        ...
        loadSelect2Options(electronicos_de_importacion, ElectronicSelectedProducts);
        console.log("Sincronización de productos onure lista");
        ...
        $selector.on('select2:select select2:unselect', function () {
            $('#hiddenTestInputElectronic').trigger("change");
        });

        console.log("Select dinámico de productos onure cargado exitosamente");
        ...
        console.log("SELECT2 INIT ONURE JS OK 200")
    });
})(jQuery);